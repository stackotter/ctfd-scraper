...
class Scraper():
    ...
    def download(self, directory_name, export_json=False):
        ...
        for chall in challenges['data']:
            ...
            cleaned_name = self.cleanString(name)
            cleaned_category = self.cleanString(category)

            print("Downloading %s (%s)" % (name, category))

            # Load challenge details
            chall_info = json.loads(self.apiGet("/challenges/%d" % id).text)['data']
